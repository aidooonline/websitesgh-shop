 ...
 */
/**
 * Direct URL of a product's featured image, or empty string.
 *
 * Used as the FIRST line of a WhatsApp order message. WhatsApp previews the
 * first url it finds, and a direct image url renders as a picture in the chat
 * without depending on og:image being read correctly, which is the most
 * reliable way to make an order look like a real product order.
 *
 * @param WC_Product|null $product Product.
 * @return string
 */
function wghs_product_image_url( $product = null ) {
...
function wghs_wa_product_message( $product = null ) {
	...
	$map = array(
		...
		'{{image}}'   => wghs_product_image_url( $product ),
...
function wghs_wa_product_button() {
	...
	<div class="wghs-wa-order">
		<a class="wghs-wa-order__btn" href="<?php echo esc_attr( $link ); ?>" target="_blank" rel="noopener"
